...
@onready var currentStamina: int = 100
...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("P2_MoveLeft"):  move_left = true
	if event.is_action_released("P2_MoveLeft"): move_left = false

	if event.is_action_pressed("P2_MoveRight"):  move_right = true
	if event.is_action_released("P2_MoveRight"): move_right = false
